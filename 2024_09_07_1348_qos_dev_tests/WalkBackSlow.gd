...
var LS_MAP = StickMap.new().LeftStick["WalkBack"]
...
#		util.debug("Un-modified Input Vector: " + str(input_vector) )
	...
	speedTarget = stickX * stats.WALK_BACK_SPEED_MAX
	## Scenarios A : 1 - 4
	### Facing Right, Not Too Fast, and Moving in a Correct Direction
	if parent.is_facing_right and ( stats.WALK_BACK_SPEED_MAX >= stats.speed_x() ) and ( parent.velocity.x <= 0 ):
	...
	if parent.is_facing_right and ( stats.speed_x() > stats.WALK_BACK_SPEED_MAX ) and ( parent.velocity.x <= 0 ):
