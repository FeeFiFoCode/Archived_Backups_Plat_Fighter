...
#
#	# Debug
#	util.debug("Input Vector is: " + str(input_vector) )
...
#	if ( stats.WALK_SPEED_MAX >= stats.speed_x() ) and stats.is_direction_correct(self):
...
#	if ( stats.speed_x() > stats.WALK_SPEED_MAX ) and stats.is_direction_correct(self):
...
#	if not stats.is_direction_correct(self):
