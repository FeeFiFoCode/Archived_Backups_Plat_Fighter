...
var LS_MAP = StickMap.new().LeftStick["WalkBackSlow"]
...
#			scaleDirFacing = -1
	...
	#			direction = 1
		...
		speedTarget = stats.WALK_BACK_SPEED_MAX * stickX
	#		print("Position:",parent.transform)
	#		print("Velocity X:",parent.velocity.x)
		
		if speedCurrent == 0:
	#		print(debugString,"Debug Check Right.1A")
			accelWalkStanding = stats.WALK_BACK_ACCEL_BASE + ( stickX * stats.WALK_BACK_ACCEL_SCALE )
