...
#		util.debug("Un-modified Input Vector: " + str(input_vector) )
	...
	if parent.is_facing_right and ( stats.IDLE_SPEED_MAX >= stats.speed_x() ) and stats.is_direction_correct(self):
	...
	if parent.is_facing_right and ( stats.speed_x() > stats.IDLE_SPEED_MAX ) and stats.is_direction_correct(self):
