...
func process_physics( delta : float ) -> State:
	...
	if LS_MAP["JumpSquat"].has_point(input_vector):
...
#	Meter Boost
	...
	if ctr < stats.DASH_FRAME_PRE:
		# Pre Checks
		if LS_MAP["Moon"].has_point(input_vector):
			flagMoon = true

		if LS_MAP["Sun"].has_point(input_vector):
			flagSun = true
		...
		if flagMoon:
			if LS_MAP["DashMoon"].has_point(input_vector):
				return stateDashMoon

			elif LS_MAP["WalkMoon"].has_point(input_vector):
				return stateWalkMoon

			elif LS_MAP["Deadzone"].has_point(input_vector):
		...
		if flagSun:
			if LS_MAP["DashSun"].has_point(input_vector):
				return stateDashSun

			elif LS_MAP["SunWalk"].has_point(input_vector):
				return stateWalkSun

			elif LS_MAP["DashSticky"].has_point(input_vector):
				return stateDashSticky
		...
		if LS_MAP["TurnSmash"].has_point(input_vector):
		...
		elif LS_MAP["Run"].has_point(input_vector):
			return stateRun
		...
		elif LS_MAP["Idle"].has_point(input_vector):
			return stateIdle
		else:
			util.debug("Dash Cooldown - but no catch")
			pass
